...
import '../functions/mobile.dart' if (dart.library.html) '../functions/web.dart';
...
class Descarga extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final data = ref.read(riverResumen);
            ...
            data: (_) => Center(
              child: BotonCallback(
                ...
                callback: () async {
                  ...
                  sheet.getRangeByIndex(1, 16).setText('Comentarios');
                  sheet.getRangeByIndex(1, 17).setText('Fecha Registro');

                  for (int i = 0; i < _.value.length; i++) {
                    sheet.getRangeByIndex(i + 2, 1).setText(_.value[i].id_lg.toString());
                    ...
                    sheet.getRangeByIndex(i + 2, 9).setText(_.value[i].Documento);
                    sheet.getRangeByIndex(i + 2, 10).setText(_.value[i].Correo);
                    sheet.getRangeByIndex(i + 2, 11).setText(_.value[i].Telefono);
                    sheet.getRangeByIndex(i + 2, 12).setText(_.value[i].TipoProblema);
                    sheet.getRangeByIndex(i + 2, 13).setText(_.value[i].SuptipoProblema);
                    sheet.getRangeByIndex(i + 2, 14).setText(_.value[i].TipoLlamada);
                    sheet.getRangeByIndex(i + 2, 15).setText(_.value[i].Sede);
                    sheet.getRangeByIndex(i + 2, 16).setText(_.value[i].Comentarios);
                    sheet.getRangeByIndex(i + 2, 17).setText(_.value[i].FechaRegistro);
                  }
